...
import { verifyKitchenCodeAction } from '../actions';
...
export default function KitchenAuth({
    ...
    async function handleKitchenAuth(prevState: any, formData: FormData) {
        ...
        const result = await verifyKitchenCodeAction(slug, code.trim());
    ...
    const [state, formAction, pending] = useActionState(handleKitchenAuth, { success: false });
                ...
                <div className="bg-white rounded-xl shadow-lg p-8">
                    <form action={formAction} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={pending}
                            className="w-full bg-orange-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                        >
                            {pending ? (
                                <>
